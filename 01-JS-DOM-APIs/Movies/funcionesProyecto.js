...
//Filtering with a dropdown menu

function filter() {

    var select = $("#slcFilter").val();
    for (var i = 0; i <= objMovies.lenth; i++) {

        if (select == "all") {
            movieFinder.append("-" + "Name:" + objMovies[i].name + "<br>" +
                    "-" + "Gender:" + objMovies[i].gender + "<br>" +
                    "-" + "Year:" + objMovies[i].year + "<br>" +
                    "-" + "Description:" + objMovies[i].description + "<br>" +
                    "-" + "Protagonists:" + objMovies[i].protagonist);
...
//Select genres from a dropdown menu(NOT FINISHED)

/*
function populate(s1) {
    var slcGenre = $("#txtMovieGenere").val();
    var s1 = document.getElementById(s1);

    var optionArray = [];
    optionArray.push(slcGenre);
